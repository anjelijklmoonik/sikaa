...
import { getAchievements, postAchivement, getAllStudents } from "@/app/api/admin";
...
export default function PencapaianSiswa() {
  ...
  const fetchStudents = async () => {
    ...
    if (Array.isArray(data)) {
      ...
      console.error("❌ Data siswa tidak dalam format array:", data);
      setStudents([]);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.studentProfilId || !form.judul || !form.tanggal || !form.deskripsi) {
    ...
    try {
      ...
      const formattedDate = new Date(form.tanggal + "T13:00:00.000Z").toISOString();
    ...
    <div className="p-6 bg-gray-100 min-h-screen">
      ...
      <div className="max-w-4xl mx-auto bg-white shadow-lg rounded-lg p-6">
        ...
        <div className="mb-6 p-4 bg-gray-50 rounded-lg shadow-md">
          <h2 className="text-lg font-semibold text-gray-800 mb-2">Tambah Pencapaian</h2>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="text-gray-700 font-semibold">Pilih Siswa:</label>
              <select name="studentProfilId" value={form.studentProfilId} onChange={handleChange} required className="w-full p-2 border rounded-md">
            ...
            <div>
              <label className="text-gray-700 font-semibold">Judul Pencapaian:</label>
              <input type="text" name="judul" value={form.judul} onChange={handleChange} required className="w-full p-2 border rounded-md" />
            </div>
            <div>
              <label className="text-gray-700 font-semibold">Tanggal:</label>
              <input type="date" name="tanggal" value={form.tanggal} onChange={handleChange} required className="w-full p-2 border rounded-md" />
            </div>
            <div className="md:col-span-2">
              <label className="text-gray-700 font-semibold">Deskripsi:</label>
              <textarea name="deskripsi" value={form.deskripsi} onChange={handleChange} required className="w-full p-2 border rounded-md" rows={3}></textarea>
            </div>
            <div className="md:col-span-2 text-center">
              <button type="submit" className="bg-yellow-500 text-white px-6 py-2 rounded-lg shadow-md hover:bg-yellow-600 transition">
        ...
        {/* Daftar Pencapaian */}
        <div className="mt-4">
          <h2 className="text-lg font-semibold text-gray-800 mb-2">Daftar Pencapaian:</h2>
            ...
            <tbody>
              {pencapaianList.map((p) => (
                <tr key={p.id} className="border-b">
                  <td className="p-2">{students.find((s) => s.id === p.studentProfilId)?.nama || "Tidak Diketahui"}</td>
