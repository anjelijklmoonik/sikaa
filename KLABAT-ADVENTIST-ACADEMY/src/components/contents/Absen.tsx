...
export default function AbsenSiswa() {
  ...
  const [absensi, setAbsensi] = useState<{
    [mapelId: number]: { tanggal: string; status: string };
  }>({});
  const [isSubmitting, setIsSubmitting] = useState<{
    [mapelId: number]: boolean;
  }>({});
  ...
  const handleSemesterChange = async (selected: any) => {
    ...
    try {
      ...
      const kelasData = jurusanData.Kelas.find(
        (k: any) => k.id === selectedKelas && k.semester === selected.value
      );
      ...
      siswa.value.MemberKelas.forEach((memberKelas: any) => {
        if (
          ...
          memberKelas.Kelas.MapelKelas.forEach((mapel: any) => {
            ...
            absensiData[mapel.id] = { tanggal: "", status: "Hadir" };
          });
        }
      });
  ...
  const handleSubmit = async (mapelId: number) => {
    ...
    try {
      ...
      const dataToSend = {
        tanggal: formatDateToYYYYMMDD(absensi[mapelId].tanggal), // ✅ Format ke YYYY-MM-DD
      ...
      toast.success(`Absensi untuk ${mapelOptions.find((m) => m.id === mapelId)?.namaMapel} berhasil disimpan.`);
    } catch (error) {
